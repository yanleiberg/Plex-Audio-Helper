...
from utils.audio_file_operations import get_audio_files, get_audio_tags, delete_file, rename_file, move_file
...
class AudioManager:
    ...
    def auto_select_duplicates(self, tree):
        ...
        for item in tree.get_children():
            ...
            if len(values) >= 10:  # 确保有足够的值
                key = (values[4].lower(), values[5].lower())  # title, artist
                duplicates[key].append((item, values[8], values[9]))  # item, length, size

        for files in duplicates.values():
            if len(files) > 1:
                files.sort(key=lambda x: (x[1], x[2]), reverse=True)  # 按长度和大小排序
                for item, _, _ in files[1:]:
    ...
    def update_track_number(self, file_path, new_track_number):
        ...
        # audio.save()
        print(f"Updated {file_path} with new track number: {new_track_number}")
    ...
    def organize_files(self, include_album):
        for root, _, files in os.walk(self.directory):
            for file in files:
                if file.lower().endswith(self.supported_formats):
                    file_path = os.path.join(root, file)
                    _, artist, album = get_audio_tags(file_path)
                    if include_album:
                        new_path = os.path.join(self.directory, artist, album, file)
                    else:
                        new_path = os.path.join(self.directory, artist, file)
